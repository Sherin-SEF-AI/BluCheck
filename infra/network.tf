...
# API and worker tasks run in private subnets; the ALB and NAT sit in public subnets.
...
# Single NAT gateway keeps cost down. For production HA, use one per AZ.
...
# VPC endpoint for S3 so worker/api traffic to S3 avoids the NAT path.
